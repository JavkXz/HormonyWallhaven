# 鸿蒙壁纸应用项目 - FirstApplication

## 项目概述

这是一个基于HarmonyOS的壁纸下载应用，名为"FirstApplication"，包名是`com.dimine.myapplication`。项目使用ArkTS/ETS语言开发，采用ArkUI框架构建用户界面。当前已完成首页开发，实现了类似小红书的图片网格布局，展示本地壁纸资源。

关键特性：
- 目标SDK：HarmonyOS 6.0.2 (API Level 22)
- 设备类型：Phone
- 开发语言：ArkTS/ETS
- 架构模式：MVVM
- UI框架：ArkUI
...
### 关键组件
- `WallpaperItem`接口：定义壁纸数据结构
- GridRow/GridCol：实现网格布局
- Image组件：显示壁纸图片
- Button组件：下载按钮
- Scroll组件：支持滚动浏览
...
项目使用`oh-package.json5`文件管理依赖：
- 运行时依赖：无特殊依赖（使用系统内置API）
- 开发依赖：`@ohos/hypium`（测试框架）、`@ohos/hamock`（模拟工具）
